...
int pindump(int offset, pin_t* pin)
{
  int                  numPorts, i, j;
  lefiGeometries*      geometry;
  ...
  // MACRO - PIN - PORT
  const char *olayer = "";
  const char *pnam = strdup(pin->name);
  double hashx = offset/ 1000.0;
  double hashy = (*(unsigned long *)pnam % 1000) / 1000.0;
  status = lefwStartMacroPinPort(NULL);
  CHECK_STATUS(status);
  status = lefwMacroPinPortLayer("M1", 0);
  CHECK_STATUS(status);
  status = lefwMacroPinPortLayerRect(hashx, hashy, hashx+0.1, hashy+0.1, 0, 0, 0, 0);
  CHECK_STATUS(status);
...
int macroEndCB(lefrCallbackType_e c, const char* macroName, lefiUserData ud) {
  checkType(c);
  // if ((long)ud != userData) dataError();
  for (int i = 0; i < macpins; i++) pindump(i*4000/macpins, pinarray+i);
